...
void Window::Destroy() noexcept {
	if (wid) {
		...
		if (listbox) {
			...
			//gtk_widget_destroy(GTK_WIDGET(wid)); TODO: unref ?
		}
		wid = nullptr;
	}
}
...
PRectangle Window::GetPosition() const {
	...
	if (wid) {
		rc.right = gtk_widget_get_width(PWidget(wid));
		rc.bottom = gtk_widget_get_height(PWidget(wid));
	}
	return rc;
}

void Window::SetPosition(PRectangle rc) {
	gtk_widget_set_size_request(PWidget(wid), rc.Width(), rc.Height());
}

namespace {

GdkRectangle MonitorRectangleForWidget(GtkWidget *wid) noexcept {
	GdkSurface* wnd = WindowFromWidget(wid);
	GdkRectangle rcScreen = GdkRectangle();
	GdkDisplay *pdisplay = gtk_widget_get_display(wid);
	GdkMonitor* monitor = gdk_display_get_monitor_at_surface(pdisplay, wnd);
...
void Window::SetCursor(Cursor curs) {
	...
	gtk_widget_set_cursor_from_name(GTK_WIDGET(GetID()), cursorName);
}
...
PRectangle Window::GetMonitorRect(Point pt) {
	GdkDisplay *pdisplay = gtk_widget_get_display(PWidget(wid));
	GdkMonitor* monitor = gdk_display_get_monitor_at_surface(pdisplay, WindowFromWidget(PWidget(wid)));
	GdkRectangle rect{};
...
public:
	...
	~ListBoxX() noexcept override {
		...
		if (widCached) {
			gtk_window_destroy(GTK_WINDOW(widCached));
...
static gboolean ButtonPress(GtkWidget *, gint nPress, gdouble x, gdouble y, gpointer p) {
	try {
		ListBoxX *lb = static_cast<ListBoxX *>(p);
		if (nPress == 2 && lb->delegate) {
...
static gboolean ButtonRelease(GtkWidget*, gint nPress, gdouble x, gdouble y, gpointer p) {
	try {
		ListBoxX *lb = static_cast<ListBoxX *>(p);
		if (nPress == 1 && lb->delegate) {
...
void ListBoxX::Create(Window &parent, int, Point, int, bool, Technology) {
	...
	wid = widCached = gtk_window_new();

	frame = gtk_frame_new(nullptr);
	gtk_window_set_child(GTK_WINDOW(wid), GTK_WIDGET(frame));

	//gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_OUT); // TODO: CSS control
	//gtk_container_set_border_width(GTK_CONTAINER(frame), 0);

	scroller = gtk_scrolled_window_new();
	//gtk_container_set_border_width(GTK_CONTAINER(scroller), 0); // TODO: CSS control
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroller),
				       GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);

	gtk_frame_set_child(GTK_FRAME(frame), GTK_WIDGET(scroller));

	// TODO: use GtkListView ?
	...
	GtkGesture* clickGest = gtk_gesture_click_new();
	gtk_gesture_single_set_button(GTK_GESTURE_SINGLE(clickGest), GDK_BUTTON_PRIMARY);
	g_signal_connect(G_OBJECT(widget), "pressed",
			 G_CALLBACK(ButtonPress), this);
	g_signal_connect(G_OBJECT(widget), "released",
			 G_CALLBACK(ButtonRelease), this);

	GtkRoot* root = gtk_widget_get_root(GTK_WIDGET(parent.GetID()));
	gtk_window_set_transient_for(GTK_WINDOW(wid), GTK_WINDOW(root));
}
...
PRectangle ListBoxX::GetDesiredRect() {
	...
	if (wid) {
		...
		gtk_style_context_get_padding(styleContextFrame, &padding);
		gtk_style_context_get_border(styleContextFrame, &border);
